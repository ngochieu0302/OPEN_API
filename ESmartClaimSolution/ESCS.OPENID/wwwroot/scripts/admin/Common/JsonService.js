function JsonService() {
    ...
    //this.ChuyenHangThanhCot = function (array, funcGroup, funcCompare, funcKey, funcValue) {
    //    if (array.length <= 0) {
    //        return arr;
    //    }
    //    var arrGroup = [];
    ...
    //        var i = funcGroup(item);
    //        if (!arrGroup.some(n => funcCompare(n, i))) {
    //            arrGroup.push(i);
    //        }
    //    });
    //    arrGroup.forEach(function (itemGroup) {
    //        var obj = itemGroup;
    //        array.forEach(function (item) {
    //            if (funcCompare(itemGroup, item)) {
    //                var arrKeyGroup = funcKey(item);
    //                var key = ""; 
    //                arrKeyGroup.forEach(function (item,index) {
    //                    if (index === 0) {
    //                        key += item.toString();
    //                    }
    //                    else {
    ...
    //                    }
    //                });
    //                obj[Base64.encode(key)] = funcValue(item);
    //            }
    //        });
    //        arrResult.push(obj);
    //    });
    //    return arrResult;
    //};
    //this.ChuyenCotThanhHang = function (array, funcGroup, arrColGroup, arrCol, colValue) {
    //    if (array.length <= 0) {
    //        return arr;
    //    }
    //    var arrResult = [];
    //    array.forEach(function (item) {
    //        for (var key in item) {
    //            var obj = funcGroup(item);
    //            if (!arrColGroup.includes(key)) {
    //                //Giải mã key
    //                var strKey = Base64.decode(key);
    //                var arrProperty = strKey.split("$");
    //                arrCol.forEach(function (propertyName,index) {
    //                    obj[propertyName] = arrProperty[index];
    //                });
    //                obj[colValue] = item[key];
    //                arrResult.push(obj);
    //            }
    //        }
    //    });
